...
function getSampleImageUrl(category, subcategory, name) {
  ...
  if (n.includes('fish') || n.includes('mach') || n.includes('meen')) return CATEGORY_IMAGES.fish;
  if (n.includes('prawn') || n.includes('crab') || n.includes('squid') || n.includes('shrimp')) return CATEGORY_IMAGES.seafood;
  if (n.includes('mutton') || n.includes('gosht') || n.includes('lamb') || n.includes('pork') || n.includes('beef')) return CATEGORY_IMAGES.mutton;
  if (n.includes('chicken') || n.includes('kodi') || n.includes('murgh')) return CATEGORY_IMAGES.chicken;
  if (n.includes('dal') || n.includes('pappu') || n.includes('sambar') || n.includes('rasam')) return CATEGORY_IMAGES.dal;
  if (n.includes('roti') || n.includes('paratha') || n.includes('naan') || n.includes('kulcha') || n.includes('poori') || n.includes('bhature')) return CATEGORY_IMAGES.roti;
  if (c.includes('sweets') || c.includes('desserts') || n.includes('halwa') || n.includes('kheer') || n.includes('laddu') || n.includes('jamun') || n.includes('payasam')) return CATEGORY_IMAGES.sweets;
  if (c.includes('drinks') || n.includes('lassi') || n.includes('chai') || n.includes('sherbet')) return CATEGORY_IMAGES.drinks;
  if (c.includes('street') || n.includes('chaat') || n.includes('puri') || n.includes('pav') || n.includes('pakora')) return CATEGORY_IMAGES.street;
  if (c.includes('chutney') || c.includes('pickle') || n.includes('pachadi')) return CATEGORY_IMAGES.chutney;
  if (n.includes('rice') || n.includes('pulao') || n.includes('bath') || n.includes('khichdi') || n.includes('pongal')) return CATEGORY_IMAGES.rice;
  return CATEGORY_IMAGES.chicken;
}

// Master Raw Data Generator
import { rawDishes } from './dishesList.js';

console.log(`Generating dataset from ${rawDishes.length} structured authentic dishes...`);

const processedRecipes = rawDishes.map((raw, index) => {
...
console.log(`Successfully generated and saved ${processedRecipes.length} authentic Indian recipes to ${batch1Path}`);
